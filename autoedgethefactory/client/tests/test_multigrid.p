...
define temp-table eDepartment no-undo before-table eDepartmentBefore
    ...
    procedure AddButtonClick:
        ...
        if valid-handle(bs1:Handle:get-buffer-handle(cTable):parent-relation) then
...
/*            hqry = bs1:Handle:get-buffer-handle(cTable):parent-relation:current-query.*/
        else
        ...
        MESSAGE 
            'ctable=' ctable skip
            'hqry:prepare-string=' hqry:prepare-string skip
            'current-query=' bs1:Handle:get-buffer-handle(cTable):parent-relation:current-query
        VIEW-AS ALERT-BOX.
